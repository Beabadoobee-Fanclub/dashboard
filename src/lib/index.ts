...
export class DiscordImage {
  ...
  static getAvatarURL = (
    ...
    if (avatar.startsWith("a_"))
      return `${DiscordImage.CDN_URL}/avatars/${user_id}/${avatar}.gif?size=${size}`;
    return `${DiscordImage.CDN_URL}/avatars/${user_id}/${avatar}.png?size=${size}`;
  };

  static getGuildMemberAvatarURL = (
    guild_id: string,
    user_id: string,
    avatar: String,
    size: number = 128
  ...
  static getGuildDiscoverySplashURL = (
    guild_id: string,
  ...
  static getGuildBannerURL = (
    guild_id: string,
    guild_banner: string,
    size: number = 128
  ) => {
    if (guild_banner.startsWith("a_"))
      return `${DiscordImage.CDN_URL}/banners/${guild_id}/${guild_banner}.gif?size=${size}`;
    return `${DiscordImage.CDN_URL}/banners/${guild_id}/${guild_banner}.png?size=${size}`;
  };

  static getDefaultImageURL = (size: number = 128) => {
    return `${DiscordImage.CDN_URL}/embed/avatars/0.png?size=${size}`;
  };
}
